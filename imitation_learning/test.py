...
def run_episode(env, agent, rendering=True, max_timesteps=1000, history_length=3):
    ...
    state = env.reset()
    state_history = torch.zeros(history_length, 96, 96).unsqueeze(0).to('cuda')
    ...
    while True:
        ...
        state = torch.tensor(rgb2gray(state)).to('cuda')
        state = state.reshape(1, 96, 96).unsqueeze(0).to('cuda')
...
if __name__ == "__main__":
    ...
    net.to('cuda')
    agent = BCAgent(network=net)
    agent.load("models/hl5.pt")
